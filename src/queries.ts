 ...
 * asignación.
 */
export async function listAllBoats(supabase: SupabaseClient, eventId: string): Promise<Boat[]> {
  const { data, error } = await k(supabase)
    .from('event_entrants')
    .select('id, event_id, class_id, club_id, full_name, sail_number, entrant_crew(*)')
    .eq('event_id', eventId)
 ...
 */
export async function listDeclarantCandidates(
  supabase: SupabaseClient,
  eventId: string,
): Promise<DeclarantCandidate[]> {
  ...
  const founderId = (eventRow as { created_by?: string } | null)?.created_by
  ...
  const byId = new Map(
    ((profileRows ?? []) as Array<{ id: string; email: string; name: string | null }>).map((p) => [p.id, p]),
  )
...
export async function listEventAssignments(
  ...
  return (data ?? []).map((row: Record<string, unknown>) => ({
    id: row.id as string,
    entrant_id: row.entrant_id as string,
 ...
 * autoría.
 */
export async function setDeclarantBoats(
  supabase: SupabaseClient,
  userId: string,
  assignedBy: string,
  currentEntrantIds: string[],
  nextEntrantIds: string[],
